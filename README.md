...
- 五節可切換且畫面、配置、操作都不同的車廂：臥室（安撫）、武器物資（百葉／維修）、工坊情報（回收零件）、溫室（種植）、廚房儲藏（熱食）。
...
以下為 v0.7.0 從同一個可玩版本直接截取的五車廂與農業／槽位畫面；不是獨立概念圖：
...
v0.7.0 的真人視角驗收不是只檢查函式：Playwright 真的在 390×844 與 360×640 瀏覽器中點擊與滑動。預設 390×844 車廂保有 534px 不被面板切斷的畫面，360×640 保有 337px；最小可點區為 54×48px、底部指令列 70px，49 種操作共通過 406 項瀏覽器斷言。詳見 [手機肉眼可玩性驗收](docs/MOBILE_VISUAL_QA.md) 與 [可公開下載的 JSON 報告](public/assets/qa/mobile-playability-report.json)。
...
全操作手機瀏覽器稽核（49 種操作、390×844／360×640 肉眼版面、左右滑動、可收合工具、五車廂、兩夜種植／澆水／收成、相容槽點放與實際拖曳、重載保存、局外預覽與破口維修）：
